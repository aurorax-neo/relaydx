...
static struct uloop_fd rtnl_sock = {.fd = -1};
...
int relayd_rtnl_init(void)
{
	...
	rtnl_sock.fd = socket(AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC, NETLINK_ROUTE);
	if (rtnl_sock.fd < 0) {
	...
	if (uloop_fd_add(&rtnl_sock, ULOOP_READ | ULOOP_EDGE_TRIGGER) < 0) {
		close(rtnl_sock.fd);
		rtnl_sock.fd = -1;
		return -1;
	}
