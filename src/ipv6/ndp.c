...
static void free_neighbor(struct ndp_neighbor *neighbor);
...
static struct relayd_event ndp_event_solicit = {
	.socket = -1,
	.handle_dgram = handle_solicit,
};
static struct relayd_event rtnl_event = {
	.socket = -1,
	.handle_dgram = handle_rtnetlink,
};
...
int init_ndp_proxy(const struct relayd_config *relayd_config)
{
	...
	if (relayd_register_event(&rtnl_event) < 0)
		goto error;
	...
	for (size_t i = 0; i < config->static_ndp_len; ++i) {
		struct ndp_neighbor *n = calloc(1, sizeof(*n));
		if (!n)
			goto error;
		n->static_entry = true;
		n->timeout = 0;
		...
		if (sscanf(config->static_ndp[i], "%45[^/]/%u:%15s", ipbuf, &prefix, iface) < 3
				|| prefix > 128 || inet_pton(AF_INET6, ipbuf, &n->addr) != 1 ||
				!(n->iface = relayd_get_interface_by_name(iface))) {
			syslog(LOG_ERR, "Invalid static NDP-prefix %s", config->static_ndp[i]);
			free(n);
			goto error;
		}
		n->len = (uint8_t)prefix;
	...
	if (relayd_register_event(&ndp_event_solicit) < 0)
		goto error;

	// Open ICMPv6 socket
	ping_socket = socket(AF_INET6, SOCK_RAW | SOCK_CLOEXEC, IPPROTO_ICMPV6);
	if (ping_socket < 0) {
		syslog(LOG_ERR, "Unable to open ICMPv6 socket: %s", strerror(errno));
		goto error;
	}
...
void deinit_ndp_proxy(void)
{
	while (!list_empty(&neighbors)) {
		struct ndp_neighbor *neighbor = list_first_entry(&neighbors,
	...
	if (ndp_event_solicit.socket >= 0)
		relayd_unregister_event(&ndp_event_solicit);
	if (rtnl_event.socket >= 0)
		relayd_unregister_event(&rtnl_event);
	config = NULL;
}
...
{
	struct ip6_hdr *ip6;
	struct nd_neighbor_solicit *req;
	struct sockaddr_ll *ll = addr;
	bool ns_is_dad;

	if (!iface || len < sizeof(*ip6) + sizeof(*req))
		return;
	ip6 = data;
		...
		struct relayd_interface *iface, bool add)
{
	...
	if (!add) { // Delete action
		if (n && !n->static_entry && (!n->iface || n->iface == iface))
			free_neighbor(n);
