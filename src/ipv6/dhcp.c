...
static struct relayd_event dhcpv6_event = {
	.socket = -1,
	.handle_dgram = handle_dhcpv6,
};

static const struct relayd_config *config = NULL;
static bool ia_initialized;
...
int init_dhcpv6_relay(const struct relayd_config *relayd_config)
{
	...
	if (dhcpv6_init_ia(relayd_config, dhcpv6_event.socket) < 0)
		goto error;
	ia_initialized = true;
	...
	if (relayd_register_event(&dhcpv6_event) < 0)
		goto error;
...
{
	if (dhcpv6_event.socket >= 0)
		relayd_unregister_event(&dhcpv6_event);
	dhcpv6_event.handle_dgram = handle_dhcpv6;
	if (ia_initialized) {
		dhcpv6_deinit_ia();
		ia_initialized = false;
	}
	config = NULL;
}
		...
		struct relayd_interface *iface)
{
	...
	ssize_t encoded_length = relayd_dns_encode_search(domain.name,
			sizeof(domain.name));
	if (encoded_length > 0) {
		domain.len = htons((uint16_t)encoded_length);
		domain_len = (size_t)encoded_length + 4;
	}
		...
		const void *data, size_t len, struct relayd_interface *iface)
{
	...
	if (h->msg_type == DHCPV6_MSG_RELAY_FORW) { // handle relay-forward
		if (len < sizeof(*h) || h->hop_count >= DHCPV6_HOP_COUNT_LIMIT)
			return; // Invalid relay message or hop count
		else
