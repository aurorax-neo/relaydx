...
static struct relayd_event reconf_event = {
	.socket = -1,
	.handle_event = reconf_timer,
};
...
int dhcpv6_init_ia(const struct relayd_config *relayd_config, int dhcpv6_socket)
{
	...
	if (relayd_register_event(&reconf_event) < 0) {
		close(reconf_event.socket);
		reconf_event.socket = -1;
		goto error;
	}
	...
	for (size_t i = 0; i < config->dhcpv6_lease_len; ++i) {
		...
		if (!duidlen || duidlen % 2 || duidlen > 260 ||
				!(assign = strtok_r(NULL, ":", &saveptr)) ||
				(saveptr && *saveptr)) {
			syslog(LOG_ERR, "Invalid static lease %s", config->dhcpv6_lease[i]);
			goto error;
		}
		for (size_t j = 0; j < duidlen; ++j) {
			if (!isxdigit((unsigned char)duid[j])) {
				syslog(LOG_ERR, "Invalid static DHCPv6 DUID");
				goto error;
			}
		}
		errno = 0;
		assigned = strtoul(assign, &endptr, 16);
		if (errno || !assign[0] || *endptr || assigned > UINT32_MAX) {
			syslog(LOG_ERR, "Invalid static DHCPv6 assignment");
			goto error;
		}
		duidlen /= 2;
...
void dhcpv6_deinit_ia(void)
{
	...
	for (size_t i = 0; i < config->slavecount; ++i) {
		...
		if (!iface->pd_assignments.next)
			continue;
		list_for_each_entry_safe(assignment, next, &iface->pd_assignments,
				head) {
			if (assignment->clid_len && assignment->length <= 64)
				apply_lease(iface, assignment, false);
			list_del(&assignment->head);
			free(assignment->hostname);
			free(assignment);
		}
		INIT_LIST_HEAD(&iface->pd_assignments);
		iface->pd_addr_len = 0;
	...
	if (reconf_event.socket >= 0)
		relayd_unregister_event(&reconf_event);
	socket_fd = -1;
	config = NULL;
}
...
static bool assign_pd(struct relayd_interface *iface, struct assignment *assign)
{
	...
	if (assign->assigned) {
		list_for_each_entry(c, &iface->pd_assignments, head) {
			...
			if (assign->assigned >= current &&
					(uint64_t)assign->assigned + asize < c->assigned) {
	...
	list_for_each_entry(c, &iface->pd_assignments, head) {
		...
		if (current + asize < c->assigned) {
			assign->assigned = (uint32_t)current;
			list_add_tail(&assign->head, &c->head);
			apply_lease(iface, assign, true);
			return true;
		}
...
static void update(struct relayd_interface *iface)
{
	...
	if (minprefix < 0)
		border->assigned = 0;
	else if (minprefix <= 32)
		border->assigned = UINT32_MAX;
	else
			...
			dhcpv6_for_each_option((uint8_t*)&ia[1], end, otype, olen, odata) {
				...
				if (a) {
					for (size_t i = 0; i < iface->pd_addr_len; ++i) {
						...
						if (!memcmp((uint8_t *)p +
								offsetof(struct dhcpv6_ia_prefix, addr),
								&addr, sizeof(addr)) && p->prefix == a->length)
								found = true;
						} else {
							addr.s6_addr32[3] = htonl(a->assigned);

						if (!memcmp((uint8_t *)n +
								offsetof(struct dhcpv6_ia_addr, addr),
								&addr, sizeof(addr)))
								found = true;
						}
					}
		...
		const struct sockaddr_in6 *addr, const void *data, const uint8_t *end)
{
	...
	dhcpv6_for_each_option(start, end, otype, olen, odata) {
		if (otype == DHCPV6_OPT_CLIENTID) {
			...
			if (relayd_dns_decode_name(odata + 1, odata + olen, odata + 1,
