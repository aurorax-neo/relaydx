...
void relayd_add_pending_route(const uint8_t *gateway, const uint8_t *dest, uint8_t mask, int timeout)
{
	...
	uloop_timeout_set(&rt->timeout, timeout);
	list_for_each_entry(rif, &interfaces, list) {
...
static void recv_packet(struct uloop_fd *fd, unsigned int events)
{
	...
	do {
		...
		if (pktlen < (int)sizeof(*pkt))
			continue;
		pkt = (void *)pktbuf;
		if (pkt->eth.ether_type != htons(ETHERTYPE_ARP) ||
				pkt->arp.arp_hln != ETH_ALEN || pkt->arp.arp_pln != 4)
			continue;
...
void relayd_forward_bcast_packet(struct relayd_interface *from_rif, void *packet, int len)
{
	...
	list_for_each_entry(rif, &interfaces, list) {
		if (rif == from_rif || rif->bcast_fd.fd < 0)
			continue;
...
static int init_interface(struct relayd_interface *rif)
{
	...
	if (uloop_fd_add(&rif->fd, ULOOP_READ | ULOOP_EDGE_TRIGGER) < 0)
		return -1;

	relayd_add_interface_routes(rif);
	rif->ipv4_routes_added = true;
	if (!forward_bcast && !forward_dhcp)
		return 0;
	...
	if (uloop_fd_add(&rif->bcast_fd,
			ULOOP_READ | ULOOP_EDGE_TRIGGER) < 0)
		return -1;
	return 0;
}
...
static void cleanup_pending_routes(void)
{
	struct relayd_pending_route *route;
	struct relayd_pending_route *next;

	list_for_each_entry_safe(route, next, &pending_routes, rt.list) {
		uloop_timeout_cancel(&route->timeout);
		list_del(&route->rt.list);
		free(route);
	}
...
static void close_ipv4_interfaces(void)
{
	...
	list_for_each_entry(rif, &interfaces, list) {
		if (rif->ipv4_routes_added) {
			relayd_del_interface_routes(rif);
			rif->ipv4_routes_added = false;
		}
		if (rif->bcast_fd.registered)
			uloop_fd_delete(&rif->bcast_fd);
		if (rif->bcast_fd.fd >= 0) {
			close(rif->bcast_fd.fd);
			rif->bcast_fd.fd = -1;
		}
		if (rif->fd.registered)
			uloop_fd_delete(&rif->fd);
		if (rif->fd.fd >= 0) {
			close(rif->fd.fd);
			rif->fd.fd = -1;
		}
...
{
	host_timeout = relayd_config->host_timeout4;
	host_ping_tries = relayd_config->host_ping_tries4;
	forward_bcast = relayd_config->forward_bcast4;
	forward_dhcp = relayd_config->forward_dhcp4;
	parse_dhcp = relayd_config->parse_dhcp4;
	route_table = relayd_config->route_table4;
	local_route_table = 0;
	memset(local_addr, 0, sizeof(local_addr));
	if (relayd_config->local_addr4_valid) {
		memcpy(local_addr, relayd_config->local_addr4, sizeof(local_addr));
		local_route_table = route_table++;
	}

	inet_sock = socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0);
	if (inet_sock < 0) {
