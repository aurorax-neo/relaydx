...
int init_router_discovery_relay(const struct relayd_config *relayd_config)
{
	...
	if (!(fp_route = fopen("/proc/net/ipv6_route", "r"))) {
		...
		router_discovery_event.socket = -1;
		return -1;
	}
	...
	if (config->enable_router_discovery_server) {
		for (size_t i = 0; i < config->slavecount; ++i) {
			...
			if (relayd_register_event(&iface->timer_rs) < 0)
				goto error;
	...
	if (config->slavecount > 0 && (config->enable_router_discovery_relay ||
			config->enable_router_discovery_server)) {
		if (relayd_register_event(&router_discovery_event) < 0)
			goto error;
	} else {
		close(router_discovery_event.socket);
		router_discovery_event.socket = -1;
...
void deinit_router_discovery_relay(void)
{
	...
	if (config->enable_router_discovery_server && fp_route) {
		in_shutdown = true;
		for (size_t i = 0; i < config->slavecount; ++i) {
			if (config->slaves[i].timer_rs.socket >= 0)
				send_router_advert(&config->slaves[i].timer_rs);
		}
	}
	for (size_t i = 0; i < config->slavecount; ++i) {
		if (config->slaves[i].timer_rs.socket >= 0)
			relayd_unregister_event(&config->slaves[i].timer_rs);
	}
	if (router_discovery_event.socket >= 0)
		relayd_unregister_event(&router_discovery_event);
	if (fp_route) {
		fclose(fp_route);
		fp_route = NULL;
	}
...
{
	if (iface->ifindex > 0)
		setsockopt(router_discovery_event.socket, SOL_SOCKET,
				SO_BINDTODEVICE, iface->ifname, strlen(iface->ifname) + 1);
}

static void unbind_icmp6_socket(void)
{
	char empty = '\0';
	setsockopt(router_discovery_event.socket, SOL_SOCKET,
			SO_BINDTODEVICE, &empty, sizeof(empty));
}
...
static void send_router_advert(struct relayd_event *event)
{
	...
	ssize_t encoded_length = relayd_dns_encode_search(domain.name,
			sizeof(domain.name));
	if (encoded_length > 0) {
		domain_len = ((size_t)encoded_length + 7) & ~(size_t)7;
		domain_len += 8;
		domain.len = (uint8_t)(domain_len / 8);
	}
	...
	relayd_urandom(&delay, sizeof(delay));
	val.it_value.tv_sec = delay % (MaxRtrAdvInterval - MinRtrAdvInterval) +
			MinRtrAdvInterval;
...
static void forward_router_advertisement(uint8_t *data, size_t len)
{
	...
	for (size_t i = 0; i < config->slavecount; ++i) {
		...
		bind_icmp6_socket(&config->slaves[i]);
		relayd_forward_packet(router_discovery_event.socket,
